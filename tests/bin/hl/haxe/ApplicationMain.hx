...
class ApplicationMain
{
	...
	{
		lime.system.System.__registerEntryPoint("Tests", create);

		#if (js && html5)
		#if (munit || (utest && openfl_enable_utest_legacy_mode))
		lime.system.System.embed("Tests", null, 800, 600);
		#end
		#else
		create(null);
		#end
	...
	macro public static function getEntryPoint()
	{
		...
		switch (Context.follow(Context.getType("Tests")))
		{
			case TInst(t, params):
				...
				if (hasMain)
				{
					return Context.parse("@:privateAccess Tests.main()", Context.currentPos());
				}
				...
				{
					Context.fatalError("Main class \"Tests\" has neither a static main nor a constructor.", Context.currentPos());
				}

			default:

				Context.fatalError("Main class \"Tests\" isn't a class.", Context.currentPos());
		}
...
@:keep @:dox(hide) class DocumentClass extends Tests {}
#else
class DocumentClass
{
